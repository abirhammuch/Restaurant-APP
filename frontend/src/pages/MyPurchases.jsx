...
const MyPurchases = () => {
  ...
  const fetchPurchasedItems = async () => {
    try {
      const response = await axios.get(`${backendUrl}/api/rating/user/purchased`, {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {purchasedItems.length === 0 ? (
        <div className="text-center py-10 bg-gray-50 rounded-lg">
          <p className="text-lg text-gray-500">You haven't purchased any items yet</p>
          <p className="text-sm text-gray-400">Order and try our delicious food!</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {purchasedItems.map((item, index) => (
            <div key={index} className="bg-white border rounded-lg p-4 shadow-sm flex items-center justify-between">
