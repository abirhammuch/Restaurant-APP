...
import { FaChevronDown, FaChevronRight, FaArrowRight, FaPlus, FaExpand } from "react-icons/fa";
    ...
    <div className='border m-9 rounded-2xl border-gray-200 shadow-2xl shadow-black pb-9'>
      ...
      </div>
      {
        userOrder && userOrder.map((order, index) => (
            <div key={index} className='grid grid-cols-[1fr_1fr_1fr_1fr] mx-9 border-b border-gray-400 px-4 py-4 mt-4'>
              <p  className='text-md font-bold'>{order._id}</p>
              <p  className={`text-md font-bold ${order.orderStatus === 'pending' ? '' : ''}`}>{order.orderStatus
                    }</p>
            ...
            scrollTo(0,0))}
              className='flex gap-2 items-center'>
                  <p  
                className='cursor-pointer  font-bold hover:text-orange-600'>View Detail 
