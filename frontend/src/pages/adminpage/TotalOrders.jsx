...
const Orders = () => {
  ...
  const filteredOrders = totalOrders?.filter(order => {
    ...
    if (searchQuery) {
      ...
      const nameMatch = order.deliveryAddress?.name?.toLowerCase().includes(query)
  ...
  // ✅ Get stats
  const getStats = () => {
    if (!totalOrders) return { total: 0, pending: 0, preparing: 0, ready: 0, delivered: 0 }
    const stats = {
      total: totalOrders.length,
      pending: totalOrders.filter(o => o.orderStatus === 'pending').length,
      preparing: totalOrders.filter(o => o.orderStatus === 'preparing').length,
      ready: totalOrders.filter(o => o.orderStatus === 'ready').length,
      delivered: totalOrders.filter(o => o.orderStatus === 'delivered').length
    }
    return stats
  }
    ...
    <div>
      ...
      <div className='flex justify-between items-center mb-5'>
        <div>
          ...
          <p className='text-gray-600'>Manage, monitor, and update live orders from the kitchen.</p>
        </div>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        ...
        <div className='border w-full flex flex-col justify-center items-center rounded-2xl py-4 border-gray-300 shadow-lg cursor-pointer hover:bg-purple-50 transition-colors'>
          ...
          <p className='font-bold text-2xl text-purple-600'>{stats.preparing}</p>
        </div>
      ...
      {/* Search & Filters */}
      <div className='grid md:grid-cols-2 gap-4 mt-6 pt-7 px-5 mb-5 rounded-2xl border border-gray-200'>
        <div className='relative'>
          ...
          />
          <img src={assets.search_icon} alt='search' className='absolute top-3 left-3 w-5 h-5 opacity-50' />
        </div>
        <div className='flex flex-wrap gap-2 justify-center md:justify-end items-center'>
          {['all', 'pending', 'confirmed', 'preparing', 'ready', 'delivering', 'delivered', 'cancelled'].map((status) => (
            <button
              key={status}
      ...
      <div className='border rounded-2xl border-gray-300 overflow-hidden'>
        ...
        {filteredOrders && filteredOrders.length > 0 ? (
          filteredOrders.map((order) => (
            <div
              key={order._id}
              className='grid grid-cols-[0.8fr_1.5fr_0.8fr_1fr_1.2fr_1.5fr_1fr_1fr] px-4 py-4 border-t border-gray-100 items-center hover:bg-gray-50 transition-colors'
            >
              <p className='text-xs font-mono truncate'>{order._id.slice(-6)}</p>
              <p className='text-sm truncate'>{order.deliveryAddress?.name || 'N/A'}</p>
              ...
              <p className='text-xs'>
                ...
                <br />
                <span className='text-gray-400 text-xs'>{getFormattedDate(order.createdAt)}</span>
              </p>
              ...
              <p className={`text-xs px-2 py-1 rounded-full text-center ${getPaymentColor(order.paymentStatus)}`}>
